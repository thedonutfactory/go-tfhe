...
func main() {
	...
	unift := tfhe.NewUniform(0, int64(Msize-1))
	...
	for i := 0; i < N; i++ {
		temp := unift.Int64() // distribution(generator);
	...
	for trial := 1; trial < 1000; trial++ {
		muT := tfhe.ModSwitchToTorus(unift.Int64(), Msize)
	...
	for trial := 1; trial < 1000; trial++ {
		...
		// MESSAGES
		muT0 := tfhe.ModSwitchToTorus(unift.Int64(), Msize)
		muT1 := tfhe.ModSwitchToTorus(unift.Int64(), Msize)
