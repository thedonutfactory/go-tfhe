...
type FFTProcessor struct {
	...
	twistiesRe []float64
	twistiesIm []float64
	planner    *gofft.Planner
	forwardFFT gofft.Fft
	inverseFFT gofft.Fft
	scratchBuf []complex128
}
...
func NewFFTProcessor(n int) *FFTProcessor {
	...
	// Create gofft planner and plans
	planner := gofft.NewPlanner()
	forwardFFT := planner.PlanForward(n2)
	inverseFFT := planner.PlanInverse(n2)
...
func (p *FFTProcessor) IFFT1024(input *[1024]params.Torus) [1024]float64 {
	...
	// Perform 512-point FFT using gofft (in-place)
	p.forwardFFT.Process(fourier)

	// Scale by 2 and convert to output
	// Rust: result[i] = fourier[i].re * 2.0;
	var result [N]float64
	for i := 0; i < N2; i++ {
		result[i] = real(fourier[i]) * 2.0
		result[i+N2] = imag(fourier[i]) * 2.0
	}
...
func (p *FFTProcessor) FFT1024(input *[1024]float64) [1024]params.Torus {
	...
	// Perform 512-point IFFT using gofft (in-place)
	p.inverseFFT.Process(fourier)

	// gofft doesn't normalize the inverse FFT, so we need to divide by N2
	// (go-dsp did this automatically)
	normFactor := 1.0 / float64(N2)
	...
	for i := 0; i < N2; i++ {
		...
		fRe := real(fourier[i]) * normFactor
		fIm := imag(fourier[i]) * normFactor
