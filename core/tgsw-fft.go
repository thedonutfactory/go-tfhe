...
import "github.com/thedonutfactory/go-tfhe/fft"
...
func tGswFFTAddH(result *TGswSampleFFT, params *TGswParams) {
	...
	for j := int32(0); j < l; j++ {
		hj := params.H[j]
		for i := int32(0); i <= k; i++ {
			result.BlocSample[i][j].A[i].AddTorusConstant(hj)
		}
	}
}
...
func tGswFFTExternMulToTLwe(accum *TLweSample, gsw *TGswSampleFFT, params *TGswParams) {
	...
	decaFFT := fft.NewLagrangeHalfCPolynomialArray(kpl, N) //fft version
	...
	for p := int32(0); p < kpl; p++ {
		fft.IntPolynomialIfft(decaFFT[p], deca[p].Coefs)
	}
