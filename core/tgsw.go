...
// Result = tGsw(0)
func TGswEncryptZero(result *TGswSample, alpha float64, key *TGswKey) {
	rlkey := &key.TlweKey
	kpl := key.Params.Kpl
	for p := int32(0); p < kpl; p++ {
 ...
 */
func TGswSymEncrypt(result *TGswSample, message *IntPolynomial, alpha float64, key *TGswKey) {
 ...
 */
func TGswSymEncryptInt(result *TGswSample, message int32, alpha float64, key *TGswKey) {
	TGswEncryptZero(result, alpha, key)
 ...
 */
func TGswEncryptB(result *TGswSample, message int32, alpha float64, key *TGswKey) {
	TGswEncryptZero(result, alpha, key)
	if message == 1 {
