...
/*
func torusPolynomialNormInftyDist(poly1 *TorusPolynomial, poly2 *TorusPolynomial) float64 {
	var norm float64 = 0
...
func torusPolynomialNormInftyDistSkipFirst(poly1 *TorusPolynomial, poly2 *TorusPolynomial) float64 {
	N := poly1.N
	var norm float64 = 0
...
func intPolynomialNorm2sq(poly *IntPolynomial) float64 {
	var norm float64 = 0
	for i := int32(0); i < poly.N; i++ {
		r := poly.Coefs[i]
		norm += float64(r * r)
...
func intPolynomialNormInftyDist(poly1 *IntPolynomial, poly2 *IntPolynomial) float64 {
	var norm float64 = 0
	// Max between the coefficients of abs(poly1-poly2)
	for i := int32(0); i < poly1.N; i++ {
...
func Multiply(a, b []int32) []int32 {
	...
	j, k, l := fft.NewLagrangeHalfCPolynomial(N), fft.NewLagrangeHalfCPolynomial(N), fft.NewLagrangeHalfCPolynomial(N)
	fft.IntPolynomialIfft(j, poly1.Coefs)
	fft.TorusPolynomialIfft(k, poly2.Coefs)
	l.Mul(j, k)
	result.Coefs = fft.TorusPolynomialFft(l)
	return result.Coefs
}
