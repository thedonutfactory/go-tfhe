...
// FFTInPlace performs in-place FFT on coefficients in Float4 format.
// Exported wrapper for use by other packages.
func FFTInPlace(coeffs []float64, tw []complex128) {
	fftInPlace(coeffs, tw)
}

// IFFTInPlace performs in-place inverse FFT on coefficients in Float4 format.
// Exported wrapper for use by other packages.
func IFFTInPlace(coeffs []float64, twInv []complex128) {
	ifftInPlace(coeffs, twInv)
}
